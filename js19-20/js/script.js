...
$('.jcarousel-pagination').on('jcarouselpagination:active', 'a', function() {$(this).addClass('active');})
                          ...
                          .jcarouselPagination({item: function(page) {return '<a href="#' + page + '">' + '</a>';} });
	
var tabContainers = $('div.tabs p');
...
function (data) {
var arr_skills =_.uniq(_.flattenDepth(_.map(data, 'skills'))).sort();
console.log('Skills:'+ arr_skills);  

var arr_names = _.map(_.sortBy(data, function (o){return o.friends.length;}), 'name');
console.log('Names:'+ arr_names); 
  
var arr_friends = _.uniq(_.map(_.flattenDepth(_.map(data,'friends')),'name'));
console.log('Friends:'+ arr_friends);   

return[arr_skills, arr_names, arr_friends];
